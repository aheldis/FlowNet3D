...
class FlowNet3D(nn.Module):
    def __init__(self, args):
        ...
        self.sa1 = PointNetSetAbstraction(npoint=8192, radius=0.5, nsample=256, in_channel=3, mlp=[32, 32, 64],
                                          group_all=False)
        self.sa2 = PointNetSetAbstraction(npoint=2048, radius=1.0, nsample=256, in_channel=64, mlp=[64, 64, 128],
                                          group_all=False)
        self.sa3 = PointNetSetAbstraction(npoint=512, radius=2.0, nsample=64, in_channel=128, mlp=[128, 128, 256],
                                          group_all=False)
        self.sa4 = PointNetSetAbstraction(npoint=256, radius=4.0, nsample=64, in_channel=256, mlp=[256, 256, 512],
                                          group_all=False)

        self.fe_layer = FlowEmbedding(radius=3.0, nsample=256, in_channel=128, mlp=[128, 128, 128], pooling='max',
                                      corr_func='concat')
